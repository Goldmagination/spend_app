...
import 'package:money_goal_tracker/src/features/goal_tracking/widgets/fullscreen_display.dart';
...
void main() {
  group('FullscreenDisplay Widget Tests', () {
    testWidgets('Displays amounts with Euro symbol (€)', (WidgetTester tester) async {
      await tester.pumpWidget(createTestableFullscreenDisplay(
      ...
      expect(find.textContaining('€126'), findsOneWidget); // toStringAsFixed(0) will round
      // Check target amount display
      expect(find.textContaining('of €501'), findsOneWidget); // toStringAsFixed(0) will round
    });

    testWidgets('Displays "GOAL ACHIEVED!" message when goalReached is true', (WidgetTester tester) async {
      await tester.pumpWidget(createTestableFullscreenDisplay(
        currentAmount: 1000,
        goalAmount: 1000,
        goalReached: true,
    ...
    testWidgets('Does not display "GOAL ACHIEVED!" message when goalReached is false', (WidgetTester tester) async {
      await tester.pumpWidget(createTestableFullscreenDisplay(
        currentAmount: 50,
        goalAmount: 100,
        goalReached: false,
    ...
    testWidgets('Clamps progress at 100% if currentAmount exceeds goalAmount', (WidgetTester tester) async {
      await tester.pumpWidget(createTestableFullscreenDisplay(
        currentAmount: 250,
        goalAmount: 200,
    ...
    testWidgets('Handles goalAmount of zero gracefully for percentage display', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(createTestableFullscreenDisplay(
        ...
        goalReached: true, // Or false, depending on how this edge case is defined for "reached"
      ));
      await tester.pumpAndSettle();
      ...
      expect(find.text('0.0%'), findsOneWidget);
      expect(find.text('GOAL ACHIEVED!'), findsNothing); // Since goalAmount is 0
    });
