...
void main() {
  ...
  group('MoneyGoalTracker Widget Tests', () {
    testWidgets('Displays "No Goal Highlighted" when no goal is highlighted', (
      WidgetTester tester,
    ) async {
      ...
      if (highlighted != null) {
        ...
        realGoalService.deleteGoal(
          tempGoal.id,
        ); //This might highlight another, need better unhighlight
      }
      ...
      expect(
        find.text('Go to Admin Panel to create or highlight a goal.'),
        findsOneWidget,
      );
      expect(
        find.widgetWithText(ElevatedButton, 'Open Admin Panel'),
        findsOneWidget,
      );
    });

    testWidgets('Displays highlighted goal details with Euro symbol', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('"Add Custom Amount" button shows dialog and adds amount', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(createTestableMoneyGoalTrackerScreen());
      await tester.pumpAndSettle();
      ...
      await tester.tap(
        find.widgetWithText(ElevatedButton, 'Add Custom Amount'),
      );
      await tester.pumpAndSettle(); // Show dialog

      // Dialog appears
      expect(
        find.text('Add Custom Amount'),
        findsNWidgets(2),
      ); // Title of dialog and button
      ...
      expect(
        find.text('€151'),
        findsOneWidget,
      ); // 150.50 rounded to 0 decimal places for display
      expect(realGoalService.getHighlightedGoal()?.currentAmount, 150.50);
    });

    testWidgets('"Add Custom Amount" dialog validates input', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(createTestableMoneyGoalTrackerScreen());
      await tester.pumpAndSettle();
    ...
    testWidgets(
      '"Add from Items" button shows bottom sheet and adds article price',
      (WidgetTester tester) async {
        ...
        // Tap "Add from Items"
        expect(
          find.widgetWithText(ElevatedButton, 'Add from Items'),
          findsOneWidget,
        );
    ...
    testWidgets('"Add from Items" button is disabled if no articles or no goal', (
      WidgetTester tester,
    ) async {
      // Case 1: No articles in the highlighted goal
      realGoalService.updateGoal(
        realGoalService.getHighlightedGoal()!.copyWith(articles: []),
      );

      await tester.pumpWidget(createTestableMoneyGoalTrackerScreen());
      await tester.pumpAndSettle();

      final addFromItemsButton = tester.widget<ElevatedButton>(
        find.widgetWithText(ElevatedButton, 'Add from Items'),
      );
