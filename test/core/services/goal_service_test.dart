...
void main() {
  group('GoalService Unit Tests', () {
    ...
    test('Initial state has one default goal if constructor logic runs for the first time', () {
      ...
      expect(defaultGoal.articles, isEmpty); // Check articles for default goal
      expect(goalService.getHighlightedGoal()?.id, defaultGoal.id);
    });

    test('Add a new goal', () {
      final goalId = generateTestId();
      final newGoal = Goal(id: goalId, name: 'Test Goal 1', targetAmount: 100, articles: []);
    ...
    test('Add multiple goals', () {
      final goal1 = Goal(id: generateTestId(), name: 'Test Goal A', targetAmount: 100, articles: []);
      final goal2 = Goal(id: generateTestId(), name: 'Test Goal B', targetAmount: 200, articles: []);
    ...
    test('Deleting a goal', () {
      ...
      final goal1 = Goal(id: goalId1, name: 'To Delete', targetAmount: 50, articles: []);
      final goalId2 = generateTestId();
      final goal2 = Goal(id: goalId2, name: 'To Keep', targetAmount: 150, articles: []);
    ...
    test('Highlighting a goal sets it as highlighted and unhighlights others', () {
      final goalId1 = generateTestId();
      final goalId2 = generateTestId();
      final goal1 = Goal(id: goalId1, name: 'Goal One', targetAmount: 100, articles: []);
      final goal2 = Goal(id: goalId2, name: 'Goal Two', targetAmount: 200, articles: []);
    ...
    test('Get highlighted goal', () {
      final goalId1 = generateTestId();
      final goal1 = Goal(id: goalId1, name: 'Highlight Me', targetAmount: 300, articles: []);
      goalService.addGoal(goal1);
      ...
      final goalId2 = generateTestId();
      final goal2 = Goal(id: goalId2, name: 'Highlight Me Instead', targetAmount: 400, articles: []);
      goalService.addGoal(goal2);
      goalService.highlightGoal(goalId2);
    ...
    test('Adding money to highlighted goal', () {
      final goalId = generateTestId();
      final goal = Goal(id: goalId, name: 'Funding Goal', targetAmount: 500, currentAmount: 50, articles: []);
    ...
    test('Resetting current amount of highlighted goal', () {
      final goalId = generateTestId();
      final goal = Goal(id: goalId, name: 'Reset Test', targetAmount: 1000, currentAmount: 250, articles: []);
      goalService.addGoal(goal);
      goalService.highlightGoal(goalId);
    ...
    test('getGoals returns correct list after operations', () {
      final goal1 = Goal(id: generateTestId(), name: 'G1', targetAmount: 10, articles: []);
      final goal2 = Goal(id: generateTestId(), name: 'G2', targetAmount: 20, articles: []);
      goalService.addGoal(goal1);
      goalService.addGoal(goal2);
    ...
    test('Deleting highlighted goal correctly updates highlighted state', () {
      final goalId1 = generateTestId();
      final goal1 = Goal(id: goalId1, name: 'Highlight Then Delete', targetAmount: 100, articles: []);
      final goalId2 = generateTestId();
      final goal2 = Goal(id: goalId2, name: 'Fallback Highlight', targetAmount: 200, articles: []);
     ...
     test('Deleting the only goal clears highlighted goal', () {
      final goalId = generateTestId();
      final goal = Goal(id: goalId, name: 'Only Goal', targetAmount: 100, articles: []);
      goalService.addGoal(goal);
      goalService.highlightGoal(goalId);
    ...
    test('Highlighting a non-existent goal does nothing', () {
      final goalId = generateTestId();
      final goal = Goal(id: goalId, name: 'Existing Goal', targetAmount: 100, articles: []);
      goalService.addGoal(goal);
      goalService.highlightGoal(goalId); 
    ...
    test('Updating a goal reflects changes including articles', () {
        ...
        Goal originalGoal = Goal(id: goalId, name: 'Original Name', targetAmount: 100, currentAmount: 10, paypalEmail: "original@test.com", articles: [article1]);
        goalService.addGoal(originalGoal);

        final article2 = Article(id: 'a2', name: 'Article 2', price: 20);
        Goal updatedGoal = Goal(id: goalId, name: 'Updated Name', targetAmount: 150, currentAmount: 20, paypalEmail: "updated@test.com", articles: [article1, article2]);
    ...
    group('Article Management in GoalService', () {
      ...
      setUp(() {
        ...
        testGoal = Goal(id: testGoalId, name: 'Goal for Articles', targetAmount: 500, articles: []);
      ...
      test('should not add an article with a duplicate ID to the same goal', () {
        final article1 = Article(id: 'artDup', name: 'Article Original', price: 30);
        final article2 = Article(id: 'artDup', name: 'Article Duplicate', price: 35);
        goalService.addArticleToGoal(testGoalId, article1);
        goalService.addArticleToGoal(testGoalId, article2); // Attempt to add duplicate
      ...
      test('should remove an article from a specific goal', () {
        final article1 = Article(id: 'artR1', name: 'Article To Remove', price: 10);
        final article2 = Article(id: 'artR2', name: 'Article To Keep', price: 20);
        goalService.addArticleToGoal(testGoalId, article1);
      ...
      test('should update an article within a specific goal', () {
        ...
        final updatedArticle = Article(id: 'artU1', name: 'Updated Name', price: 55);
      ...
      test('should not affect other goals when modifying articles for one goal', () {
        final otherGoalId = generateTestId();
        final otherGoalArticle = Article(id: 'otherArt', name: 'Other Goal Article', price: 100);
        final otherGoal = Goal(id: otherGoalId, name: 'Another Goal', targetAmount: 1000, articles: [otherGoalArticle]);
        goalService.addGoal(otherGoal);

        final articleForTestGoal = Article(id: 'artForTest', name: 'Article for Main Test Goal', price: 10);
      ...
      test('should handle trying to remove/update a non-existent article gracefully', () {
        final article = Article(id: 'artE1', name: 'Existing Article', price: 10);
        goalService.addArticleToGoal(testGoalId, article);

        goalService.removeArticleFromGoal(testGoalId, 'nonExistentArtId'); // Try removing non-existent
        final goalAfterRemove = goalService.getGoalById(testGoalId);
        expect(goalAfterRemove?.articles.length, 1); // Should not have changed

        final nonExistentUpdateArticle = Article(id: 'nonExistentArtId', name: 'Wont Update', price: 1);
        goalService.updateArticleInGoal(testGoalId, nonExistentUpdateArticle); // Try updating non-existent
        final goalAfterUpdate = goalService.getGoalById(testGoalId);
        expect(goalAfterUpdate?.articles.first.name, 'Existing Article'); // Should not have changed
      });
    });
  });
}
