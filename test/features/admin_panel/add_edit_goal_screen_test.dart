...
Widget createTestableAddEditScreen({
...
void main() {
  ...
  late GoalService
  realGoalService; // Use real service for stateful interactions
  ...
  group('AddEditGoalScreen Basic Fields and Validation', () {
    testWidgets('Renders form fields correctly with Euro symbol', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        createTestableAddEditScreen(child: AddEditGoalScreen()),
      );
      ...
      expect(
        find.widgetWithText(TextFormField, 'PayPal Email (Optional)'),
        findsOneWidget,
      );
      expect(
        find.widgetWithText(ElevatedButton, 'Save New Goal'),
        findsOneWidget,
      ); // Default for add mode
    });

    testWidgets('Entering text updates form fields', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        createTestableAddEditScreen(child: AddEditGoalScreen()),
    ...
    testWidgets('Validation errors show for empty required fields', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        createTestableAddEditScreen(child: AddEditGoalScreen()),
    ...
    testWidgets('Validation error for invalid target amount', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        createTestableAddEditScreen(child: AddEditGoalScreen()),
  ...
  group('AddEditGoalScreen Article Management', () {
    testWidgets(
      'displays empty articles list and "Add Article" button in add mode',
      (WidgetTester tester) async {
        ...
        expect(
          find.text('No articles added yet. Click "Add Article" to start.'),
          findsOneWidget,
        );
        expect(
          find.widgetWithIcon(OutlinedButton, Icons.add_shopping_cart_outlined),
          findsOneWidget,
        );
      },
    );

    testWidgets('tapping "Add Article" shows add article dialog', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        createTestableAddEditScreen(child: AddEditGoalScreen()),
      ...
      expect(
        find.widgetWithText(TextFormField, 'Article Name'),
        findsOneWidget,
      );
    ...
    testWidgets(
      'add article dialog validates and saves new article to UI list',
      (WidgetTester tester) async {
        await tester.pumpWidget(
          createTestableAddEditScreen(child: AddEditGoalScreen()),
        );
    ...
    testWidgets('displays articles from goalToEdit in edit mode', (
      WidgetTester tester,
    ) async {
      ...
      await tester.pumpWidget(
        createTestableAddEditScreen(
          child: AddEditGoalScreen(goalToEdit: existingGoal),
        ),
      );
      await tester.pumpAndSettle();
      ...
      expect(
        find.widgetWithText(ElevatedButton, 'Update Goal'),
        findsOneWidget,
      );
    });

    testWidgets(
      'tapping edit on an article shows dialog with pre-filled data',
      (WidgetTester tester) async {
        final articleToEdit = Article(
          id: 'a1',
          name: 'Old Book Name',
          price: 15,
        );
        ...
        expect(
          find.widgetWithText(TextFormField, 'Old Book Name'),
          findsOneWidget,
        );
        expect(
    ...
    testWidgets('editing an article updates it in the UI list', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('tapping remove on an article removes it from the UI list', (
      WidgetTester tester,
    ) async {
      ...
      await tester.tap(
        find.byTooltip('Remove Article').first,
      ); // Tap remove on the first article
      await tester.pumpAndSettle();
  ...
  group('AddEditGoalScreen Saving Logic', () {
    ...
    testWidgets(
      'Tapping "Save New Goal" with valid data (and articles) saves to service and pops route',
      (WidgetTester tester) async {
        ...
        final savedGoal = goals.firstWhere(
          (g) => g.name == 'Goal With Articles',
        );
    ...
    testWidgets(
      'Tapping "Update Goal" with changes (including articles) saves to service and pops route',
      (WidgetTester tester) async {
        ...
        realGoalService.addGoal(
          goalToEdit,
        ); // Add to service so it can be "updated"

        await tester.pumpWidget(
          MaterialApp(
            home: AddEditGoalScreen(goalToEdit: goalToEdit),
            navigatorObservers: [mockObserver],
          ),
        );
        await tester.pumpAndSettle();
        ...
        // Edit main goal form
        await tester.enterText(
          find.widgetWithText(TextFormField, 'Original Goal Name'),
          'Updated Goal Name',
        );
        await tester.enterText(
          find.widgetWithText(TextFormField, '50.00'),
          '75.00',
        );
